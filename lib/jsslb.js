...
const continuedLineIndentAmount = 2 // TODO make configurable
...
function doProcessLine(line, textWidth, debug) {
  ...
  while (workingCopy) {
    ...
    const continuedLineIndent = resultLines.length
      ? ' '.repeat(continuedLineIndentAmount)
      : ''
    const isShortEnoughForFinalLine =
      indentText.length +
        continuedLineIndent.length +
        workingCopy.length +
        lengthOfExtraCharsOnLastLine <=
      textWidth
    if (isShortEnoughForFinalLine) {
      debug('Remaining text is short enough for final line')
      resultLines.push(
        `${indentText}${continuedLineIndent}${usedQuote}${workingCopy}${usedQuote}${
          isTrailingComma ? ',' : ''
        }`,
      )
      workingCopy = null
    ...
    const moreCharsRepresentingQuotesAndStuff =
      ' '.repeat(lengthOfExtraCharsOnContinuedLines) + continuedLineIndent
    ...
    resultLines.push(
      `${indentText}${continuedLineIndent}${usedQuote}${contentThatFits}${usedQuote} +`,
    )
